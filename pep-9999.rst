...
become slightly more verbose, since the target scope has to be specified
explicitly.

Updating a module global::

    if any((comment := line).startswith('#') for line in lines given global comment):
        print("First comment:", comment)
    else:
        print("There are no comments")

    if all((nonblank := line).strip() == '' for line in lines given global nonblank):
...
Updating a function local::
    ...
    comment = _no_lines = object()
    if any((comment := line).startswith('#') for line in lines given nonlocal comment):
        print("First comment:", comment)
    else:
        print("There are no comments")

    nonblank = _no_lines = object()
    if all((nonblank := line).strip() == '' for line in lines given nonlocal nonblank):
        print("All lines are blank")
    else:
        print("First non-blank line:", nonblank)
...
It's the view of this PEP's author that the extra verbosity in these cases makes
it *much* clearer to a reader what's actually going on, without requiring them
to make the intuitive leap that the name binding inside the generator expression
or comprehension is going to implicitly leak into the current scope.

In the function scope case, the need to explicitly declare the target local
variable first encourages handling the case where the generator expression or
comprehension never iterates at all, and hence the inline assignment expression
never gets executed.
...
The argument against allowing this is that this code could already be more
clearly written as::
